...
if isfunctionhooked then
    if isfunctionhooked(FireServer) or isfunctionhooked(UnreliableFireServer) or isfunctionhooked(InvokeServer) and LPH_OBFUSCATED then
        return Services.LocalPlayer:Kick("Spark | Security : You are running another script, please disable it and execute again")
    end
end
...
local window = library:window({name = "bor", suffix = "os", gameInfo = string.format("Spark : %s", Game_Name:lower())})
